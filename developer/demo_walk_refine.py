...
def algorithm():
    ...
    obs_scale_lin_vel = 1.00
    obs_scale_ang_vel = 0.25
    obs_scale_gravity = 1.00
    obs_scale_dof_pos = 1.00
    obs_scale_dof_vel = 0.10
    obs_scale_action = 1.00

    obs_scale_commands = numpy.array([obs_scale_lin_vel, obs_scale_lin_vel, obs_scale_ang_vel])
    obs_scale_commands = torch.from_numpy(obs_scale_commands).float()

    obs_buf = torch.cat([
        torch_commands * obs_scale_commands,
        torch_base_measured_angular_velocity * obs_scale_ang_vel,
        torch_base_project_gravity * obs_scale_gravity,
        torch_measured_position_offset_for_policy * obs_scale_dof_pos,
        torch_joint_measured_velocity_for_policy * obs_scale_dof_vel,
        torch_action * obs_scale_action,
    ], dim=-1)
