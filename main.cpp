...
int main()
{
    ...
    std::string path = "../model.json";

    if (estimator.init(path, 0.0025))
    {
        std::cout << "Estimator initialized successfully." << std::endl;
    }
    else
    {
        std::cerr << "Failed to initialize the estimator." << std::endl;
        return -1;
    }

    // Open the file containing motor data
    std::string input_path = "../inputdata.txt";
    std::ifstream motorDataFile(input_path);
    std::string line;
    ...
    Eigen::MatrixXd estState = Eigen::VectorXd::Zero(6);
    std::vector<MotorData> motordata = readMotorData(input_path);
    ...
    while (getline(motorDataFile, line))
    {
        ...
        estState = estimator.get_base_state();

        // Print the results
        std::cout << "-------------------------------OUTPUT-------------------------------" << std::endl;
        // std::cout << "Estimated state: " << std::endl << estState << std::endl;
        std::cout << "Time: " << time << " s" << std::endl;
        std::cout << "Base Height: " << estState(2) << std::endl;
        std::cout << "Base Velocity: " << estState.block(3, 0, 3, 1) << std::endl;
        std::cout << std::endl;
