...
def demo_task():
    # 切换为开发者模式
    control_system.developer_mode(
        servo_on=True,
        control_frequency=1 / fourier_grx.gl_config.parameters.get("robot", {}).get("control_period", 0.02)
    )
...
def algorithm():
    control_system.robot_control_set_task_command(
        task_command=fourier_grx.TaskCommand.TASK_READY_STATE
    )
